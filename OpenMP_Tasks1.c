/* OpenMP Tasks --> 1st Version --> One Trial Per Task */
...
int main(int argc, char *argv[])
{
	...
	{	
        ...
        generatorbuf[0] = 0;
		generatorbuf[1] = 0; 
		double lbest_fx; //the local minimum fx of every task
        double lbest_jj; //the jj of the local minimum fx 
		#pragma omp single nowait /*single region. Only one of the threads that we made, will gonna execute the code of this region*/
		{
		    ...
		    {
		        //construction of tasks. One task per trial	
                #pragma omp task private(startpt,endpt,lbest_jj,lbest_fx) firstprivate(generatorbuf, trial) shared(best_fx, best_jj, best_trial, best_pt) 
                {

		            generatorbuf[2] = trial + omp_get_thread_num();
                    unsigned long lfevals = 0; //stores the number of function evaluations of this task
			        for (i = 0; i < nvars; i++) 
			            startpt[i] = 10.0*erand48(generatorbuf)-5.0;
       
	                lbest_jj = hooke(nvars, startpt, endpt, rho, epsilon, itermax,&lfevals);
                    lbest_fx = f(endpt, nvars);
                    lfevals++;
